...
setup_logging(logs_path, "random_forest_pdi.log")
logging.info("Starting the Random Forest model training for predicting PDI...".upper())
...
# 2. Modeling
######################################################################################################
features = DATA.drop(columns=["PDI"])
targets = DATA[["PDI"]]
...
# Save Model
model_path = os.path.join(models_path, "random_forest_pdi.pkl")
...
validation_features = VALIDATION_DATA.drop(columns=["PDI"], errors="ignore")
