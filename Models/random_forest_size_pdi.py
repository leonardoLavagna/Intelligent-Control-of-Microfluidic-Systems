...
setup_logging(logs_path, f"random_forest_multi_target.log")
logging.info("STARTING THE RANDOMFOREST MODEL TRAINING PROCESS FOR PREDICTING SIZE AND PDI...")
...
def run_model(seed, best_model_info):
    logging.info(f"STARTING THE RANDOMFOREST MODEL TRAINING PROCESS FOR SEED {seed}...")
    ...
    if train_r2 > best_model_info["best_train_r2"] and validation_r2 > best_model_info["best_val_r2"]:
        best_model_info["best_val_r2"] = test_r2
...
if best_model_info["best_model"] is not None:
    model_save_path = os.path.join(models_path, f"best_random_forest_multi_target_{file_name}.pkl")
